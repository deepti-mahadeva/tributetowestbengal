...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(!isMobileTablet() ) {
        root.classList.add("apg-desktop");
    } else {
        root.classList.remove("apg-desktop");
    }
    setHeaderWidth();
    document.body.classList.remove("apg-body-loading");
    ...
    if(modalHideBtns) {
        modalHideBtns.forEach( (btn) => {
            btn.addEventListener("click" , () => {
                let modal = btn.parentNode;

                while (modal && modal.classList && !modal.classList.contains("apg-modal")) {
    ...
    function setHeaderWidth() {
        const parallaxContainer = document.querySelector(".apg-parallax-container");
        const desktopHeaderEffectDiv = document.querySelector(".apg-header-effect");
        const desktopHeaderMain = document.querySelector(".apg-header-main");

        if(desktopHeaderEffectDiv && parallaxContainer) {
            desktopHeaderEffectDiv.style.width = parallaxContainer.clientWidth;
        }

        if(desktopHeaderMain && parallaxContainer) {
            desktopHeaderMain.style.width = parallaxContainer.clientWidth;
        }
    }

    window.addEventListener("resize", setHeaderWidth, false);
});